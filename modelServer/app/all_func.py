...
def get_dict_for_retrieval(summairzed_text, image_paths, encoded_images):
    ret_dict = {}
    ret_dict['summarizedExampleText'] = summairzed_text
    ret_dict['content'] = []
...
def get_dict_for_generation(summarized_texts, encoded_images):
    ...
    for idx in range(len(encoded_images)):
        ret_dict['generatedItems'].append({"index": idx+1, "promptText": summarized_texts[idx], "generatedImage": encoded_images[idx]})
